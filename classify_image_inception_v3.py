...
def run_inference_on_image(images,csv_dir):
  ...
  for image in images:
    ...
    with tf.Session() as sess:
      ...
      probability = [1/(1+math.exp(-x)) for x in predictions]
      #print(probability)
      csv_file = open(csv_dir,'a',newline='')
      writer = csv.writer(csv_file)
      writer.writerow([image.split('/')[-1],'%.6f'%probability[1]])
      csv_file.close()
      ...
      #for node_id in top_k:
        ...
        #print('%s:   %s (score = %.5f)' % (image.split('/')[-1],human_string, score))


def main(_):
  csv_dir = '/home/fangsh/tianchi/tianchi_dataset/submit.csv'
  image_dir = FLAGS.image_file
  images = glob.glob(image_dir+'/*.jpg')
  run_inference_on_image(images,csv_dir)
